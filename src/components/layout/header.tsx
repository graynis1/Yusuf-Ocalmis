    ...
    <header className="sticky top-0 z-40 border-b border-[var(--border)] bg-white/90 backdrop-blur">
      ...
      {categories.length > 0 && (
        <div className="border-t border-[var(--border)] bg-white">
          <nav className="container flex items-center gap-1 overflow-x-auto py-2 text-sm">
            {categories.map((c) => (
              <Link
                key={c.slug}
                href={`/kategori/${c.slug}`}
                className="whitespace-nowrap rounded-md px-3 py-1.5 text-[var(--muted)] hover:bg-[var(--surface)] hover:text-ink"
              >
                {c.name}
              </Link>
            ))}
